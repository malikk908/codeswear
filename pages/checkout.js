...
const Checkout = ({ cart, addToCart, removeFromCart, clearCartAfterCheckout, subTotal }) => {
  ...
  return (
    <div className='container px-10 sm:px-6 sm:m-auto  text-gray-600 dark:text-gray-200'>
      ...
      <div className="mt-5 flex flex-col sm:flex-row">
        ...
        <div className="px-2 w-full">
          <div className="relative mb-4">
            ...
            <input {...(session?.user ? {readOnly:true} : {onChange: handleChange})} value={email} type="email" id="email" name="email" className="w-full bg-white dark:text-gray-300  dark:bg-gray-800 rounded border border-gray-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out" />          
      ...
      <div className='flex p-6 my-4 justify-center sm:justify-start'>
      <button disabled={disabled || Object.keys(cart).length == 0} onClick={handleCheckout} className={`flex items-center w-full sm:w-auto justify-center text-white bg-pink-500 border-0 py-2 px-2 focus:outline-none ${!disabled || Object.keys(cart).length ? `hover:bg-pink-600` : 'disabled:opacity-50'}  rounded text-sm`} type="submit" role="link"><BsFillBagCheckFill className='m-1 mr-2' />
          Pay Rs.{subTotal}
        </button>
      </div>
